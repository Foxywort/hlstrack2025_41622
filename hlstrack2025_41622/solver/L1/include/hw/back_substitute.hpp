...
struct backSubstituteTraits {
    ...
    static const int DIAG_II = 1;  // Specify the pipelining target for the diag loop in backSubstituteAlt - keep at 1 for critical path
};
...
#pragma HLS PIPELINE II = 1
                    ...
                    if (k >= j) {
                        ...
                        if (i > k) {
                            ...
                            typename BackSubstituteTraits::PROD_T A_kj_reg;
                            typename BackSubstituteTraits::PROD_T B_ik_reg;
                            A_kj_reg = A[k][j]; // For fixed-point, re-size to higher precision of B
                            B_ik_reg = B[i][k];
                            subst_prod = A_kj_reg * B_ik_reg;
                            subst_prod_sum = subst_prod; // Resize
#pragma HLS EXPRESSION_BALANCE
                            subst_sum += subst_prod_sum;
                        }
                    }
                    // Use intermediate register to reduce combinational logic
                    ...
                    // but halves
                    // the DSP48 usage.
                    typename BackSubstituteTraits::OUTPUT_T neg_diag_prod_temp = -subst_sum * diag_recip_low;
                    B[i][j] = neg_diag_prod_temp;
                } // end i>=j
            }
        }
    }
...
#pragma HLS BIND_STORAGE variable = diag type = RAM_2P impl = LUTRAM latency = 1
...
#pragma HLS ARRAY_PARTITION variable = row_sum cyclic factor = 2 dim = 2
#pragma HLS BIND_STORAGE variable = row_sum type = RAM_2P impl = LUTRAM latency = 1

    typename BackSubstituteTraits::ADD_T diag_recip_low;
    typename BackSubstituteTraits::MULTNEG_T neg_diag_prod;
    OutputType select_column_multiplier;
    OutputType column_multiplier[RowsColsA];
#pragma HLS ARRAY_PARTITION variable = column_multiplier cyclic factor = 2 dim = 1
#pragma HLS BIND_STORAGE variable = column_multiplier type = RAM_2P impl = LUTRAM latency = 1
    OutputType subst_prod_m1;
