...
#pragma HLS STREAM variable = matrixUStrm depth = 64
    hls::stream<typename CholeskyInverseTraits::BACK_SUBSTITUTE_OUT> matrixInverseUStrm;
#pragma HLS STREAM variable = matrixInverseUStrm depth = 64
    int U_singular;
    ...
    hls::stream<typename CholeskyInverseTraits::CHOLESKY_OUT> matrixUStrm_delayed;
#pragma HLS STREAM variable = matrixUStrm_delayed depth = 64
    
    // Delay stage to reduce combinational logic
    delay_stage: for (int i = 0; i < RowsColsA * RowsColsA; i++) {
#pragma HLS PIPELINE II = 1
        typename CholeskyInverseTraits::CHOLESKY_OUT temp_val;
        temp_val = matrixUStrm.read();
        matrixUStrm_delayed.write(temp_val);
    }
    ...
    backSubstitute<RowsColsA, typename CholeskyInverseTraits::CHOLESKY_OUT,
                   ...
                   typename CholeskyInverseTraits::BACK_SUBSTITUTE_TRAITS>(matrixUStrm_delayed, matrixInverseUStrm, U_singular);
