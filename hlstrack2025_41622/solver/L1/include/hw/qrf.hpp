...
struct qrfTraits {
    ...
    static const int UPDATE_II = 2;   // Specify the pipelining target for the Q & R update loops - reduced for lower latency
    static const int UNROLL_FACTOR =
        4; // Specify the unrolling factor for Q & R update loops of the QRF_ALT architecture(1) - increased for better throughput with BRAM
};
...
void qrf_alt(hls::stream<InputType>& matrixAStrm,
    ...
    // Internal array memories with optimized memory binding for timing
...
#pragma HLS BIND_STORAGE variable = q_i type = RAM_2P impl = LUTRAM latency = 1
#pragma HLS BIND_STORAGE variable = r_i type = RAM_2P impl = LUTRAM latency = 1
...
#pragma HLS UNROLL FACTOR = QRF_TRAITS::UNROLL_FACTOR
                ...
                // Pre-fetch values to reduce memory access delay
                OutputType r1_val = r_i[rot_row1][k];
                OutputType r2_val = r_i[rot_row2][k];
                qrf_mm_or_mag(G_delay, r1_val, r2_val, mag_delay, use_mag_reg, extra_pass2);
            }
        update_q:
            for (int k = 0; k < RowsA; k++) {
#pragma HLS PIPELINE II = 1
#pragma HLS UNROLL FACTOR = QRF_TRAITS::UNROLL_FACTOR
                // Pre-fetch values to reduce memory access delay
                OutputType q1_val = q_i[rot_row1][k];
                OutputType q2_val = q_i[rot_row2][k];
                qrf_mm(G_delay, q1_val, q2_val);
            }
        }
    }
