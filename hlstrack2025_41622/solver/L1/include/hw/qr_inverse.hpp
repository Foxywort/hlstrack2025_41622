...
#pragma HLS DATAFLOW
    // Define intermediate buffers with optimized depth for better throughput with BRAM
    hls::stream<typename QRInverseTraits::InternalType> matrixQStrm;
#pragma HLS STREAM variable = matrixQStrm depth = 64
    hls::stream<typename QRInverseTraits::InternalType> matrixRStrm;
#pragma HLS STREAM variable = matrixRStrm depth = 64
    hls::stream<typename QRInverseTraits::InternalType> matrixInverseRStrm;
#pragma HLS STREAM variable = matrixInverseRStrm depth = 64
    ...
    // Add intermediate delay stage to improve timing
    hls::stream<typename QRInverseTraits::InternalType> matrixRStrm_delayed;
#pragma HLS STREAM variable = matrixRStrm_delayed depth = 64
    
    // Delay stage to reduce combinational logic
    delay_stage: for (int i = 0; i < RowsColsA * RowsColsA; i++) {
#pragma HLS PIPELINE II = 1
        typename QRInverseTraits::InternalType temp_val;
        temp_val = matrixRStrm.read();
        matrixRStrm_delayed.write(temp_val);
    }
    ...
    backSubstitute<RowsColsA, typename QRInverseTraits::InternalType, typename QRInverseTraits::InternalType,
                   typename QRInverseTraits::BACK_SUB_CONFIG>(matrixRStrm_delayed, matrixInverseRStrm, A_singular);
