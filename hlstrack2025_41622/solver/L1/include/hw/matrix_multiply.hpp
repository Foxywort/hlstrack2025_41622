...
struct matrixMultiplyTraits {
    ...
    static const int UNROLL_FACTOR = 4; // Specify the inner loop unrolling factor - increased for better throughput with BRAM
};
...
#pragma HLS BIND_STORAGE variable = B type = RAM_2P impl = LUTRAM latency = 1
#pragma HLS BIND_STORAGE variable = C type = RAM_2P impl = LUTRAM latency = 1
...
#pragma HLS UNROLL FACTOR = MULTIPLIER_TRAITS::UNROLL_FACTOR
                ...
                typename MULTIPLIER_TRAITS::INPUT_T A_val_reg;
                typename MULTIPLIER_TRAITS::INPUT_T B_val_reg;
                A_val_reg = GetMatrixElement<TransposeFormA, RowsA, ColsA, InputType>(A, r, k);
                B_val_reg = GetMatrixElement<TransposeFormB, RowsB, ColsB, InputType>(B, k, c);
                
                // Multiply with intermediate register
                mult = A_val_reg * B_val_reg;
                
                // Cast mult to the correct output size before adding - use intermediate register
                recast_mult = mult;
                
                // Sum with intermediate register to reduce combinational logic
                typename MULTIPLIER_TRAITS::ACCUM_T sum_temp;
                if (k == 0) {
                    // Initialise
                    sum_mult[c] = recast_mult;
                } else if (k == MULTIPLIER_TRAITS::ColsATrans - 1) {
                    // Store result with intermediate register
                    sum_temp = sum_mult[c] + recast_mult;
                    C[r][c] = sum_temp;
                } else {
                    // Accumulate with intermediate register
                    sum_temp = sum_mult[c] + recast_mult;
                    sum_mult[c] = sum_temp;
                }
            }
...
#pragma HLS BIND_STORAGE variable = B type = RAM_2P impl = LUTRAM latency = 1
#pragma HLS BIND_STORAGE variable = C type = RAM_2P impl = LUTRAM latency = 1
#pragma HLS BIND_STORAGE variable = sum_mult type = RAM_2P impl = LUTRAM latency = 1
