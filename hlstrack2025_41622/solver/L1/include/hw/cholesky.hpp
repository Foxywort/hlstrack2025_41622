...
struct choleskyTraits {
    ...
    static const int ARCH =
        2; // Select implementation: 0=Basic, 1=Lower latency architecture, 2=Further improved latency architecture - use best architecture
    static const int INNER_II = 1; // Specify the pipelining target for the inner loop
    static const int UNROLL_FACTOR =
        4; // Specify the inner loop unrolling factor for the choleskyAlt2 architecture(2) to increase throughput - increased for better performance with BRAM
...
#pragma HLS PIPELINE II = 1
    ...
    T_IN sqrt_result;
    sqrt_result = x_sqrt(a);
    dout.real(sqrt_result);
    return (0);
}
template <typename T_IN, typename T_OUT>
...
#pragma HLS PIPELINE II = 1
    CType temp_real, temp_imag;
#pragma HLS BIND_OP variable=temp_real op=mul impl=dsp
#pragma HLS BIND_OP variable=temp_imag op=mul impl=dsp
    temp_real = A.real() * B;
    temp_imag = A.imag() * B;
    C.real(temp_real);
    C.imag(temp_imag);
...
#pragma HLS BIND_STORAGE variable = sum type = RAM_2P impl = LUTRAM latency = 1
...
#pragma HLS ARRAY_PARTITION variable = L_internal block factor = 2 dim = 2
#pragma HLS BIND_STORAGE variable = L_internal type = RAM_2P impl = LUTRAM latency = 1
...
int choleskyAlt(const InputType A[RowsColsA][RowsColsA], OutputType L[RowsColsA][RowsColsA]) {
    ...
    // Optimize internal memories with better memory binding for timing
...
#pragma HLS BIND_STORAGE variable = diag_internal type = RAM_2P impl = LUTRAM latency = 1
...
row_loop:
    for (int i = 0; i < RowsColsA; i++) {
#pragma HLS PIPELINE II = 1
        // Index generation for optimized/packed L_internal memory - pre-calculate to reduce combinational logic
        int i_sub1 = i - 1;
...
#pragma HLS loop_tripcount max = 1 + RowsColsA / 2
            ...
            // Prime the off-diagonal sum with target elements A value - use intermediate registers
            typename CholeskyTraits::ACCUM_T A_val_reg;
            if (LowerTriangularL == true) {
                A_val_reg = A[i][j];
            } else {
                A_val_reg = hls::x_conj(A[j][i]);
            }
            product_sum = A_val_reg;
            
        sum_loop:
            for (int k = 0; k < j; k++) {
#pragma HLS loop_tripcount max = 1 + RowsColsA / 2
#pragma HLS PIPELINE II = 1
                // Use intermediate registers to reduce combinational logic
                typename CholeskyTraits::PROD_T L_i_k_reg;
                typename CholeskyTraits::PROD_T L_j_k_conj_reg;
                L_i_k_reg = L_internal[i_off + k];
                L_j_k_conj_reg = hls::x_conj(L_internal[j_off + k]);
                // Bind complex multiplication to DSP for lower latency
#pragma HLS BIND_OP variable=prod op=mul impl=dsp
                prod = -L_i_k_reg * L_j_k_conj_reg;
                prod_cast_to_sum = prod;
#pragma HLS EXPRESSION_BALANCE
            ...
            // Bind complex conjugate multiplication to DSP for lower latency
            typename CholeskyTraits::ACCUM_T conj_mult_result;
#pragma HLS BIND_OP variable=conj_mult_result op=mul impl=dsp
            conj_mult_result = hls::x_conj(new_L) * new_L;
#pragma HLS EXPRESSION_BALANCE
            square_sum += conj_mult_result;
            // Store result
...
int choleskyAlt2(const InputType A[RowsColsA][RowsColsA], OutputType L[RowsColsA][RowsColsA]) {
    ...
    // Optimize memory binding for better timing
#pragma HLS BIND_STORAGE variable = square_sum_array type = RAM_2P impl = LUTRAM latency = 1
#pragma HLS BIND_STORAGE variable = product_sum_array type = RAM_2P impl = LUTRAM latency = 1
...
#pragma HLS EXPRESSION_BALANCE
                        ...
                        // Build sum for use in diagonal calculation for this row.
                        // Bind complex conjugate multiplication to DSP for lower latency
                        typename CholeskyTraits::ACCUM_T conj_mult_result_alt2;
#pragma HLS BIND_OP variable=conj_mult_result_alt2 op=mul impl=dsp
                        conj_mult_result_alt2 = hls::x_conj(new_L) * new_L;
                        if (k == 0) {
                            square_sum_array[j] = conj_mult_result_alt2;
                        } else {
                            square_sum_array[j] = conj_mult_result_alt2;
                        }
                        // Store result
...
#pragma HLS BIND_STORAGE variable = A type = RAM_2P impl = LUTRAM latency = 1
    OutputType L[RowsColsA][RowsColsA];
#pragma HLS ARRAY_PARTITION variable = L cyclic factor = 2 dim = 2
#pragma HLS BIND_STORAGE variable = L type = RAM_2P impl = LUTRAM latency = 1
