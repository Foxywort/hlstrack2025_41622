...
struct svdTraits {
    ...
    static const int OFF_DIAG_II = 4; // Specify the pipelining target for the off diagonal loop. Upto 4 memory accesses
                                      // on single array in one iteration, use mulitple - reduced for lower latency
    static const int DIAG_II = 4; // Specify the pipelining target for the diagonal loop. >1 enables reuse of operators - reduced for lower latency
...
#pragma HLS BIND_STORAGE variable = s_col1 type = RAM_2P impl = LUTRAM latency = 1
#pragma HLS BIND_STORAGE variable = s_col2 type = RAM_2P impl = LUTRAM latency = 1
#pragma HLS BIND_STORAGE variable = u_col1 type = RAM_2P impl = LUTRAM latency = 1
#pragma HLS BIND_STORAGE variable = u_col2 type = RAM_2P impl = LUTRAM latency = 1
#pragma HLS BIND_STORAGE variable = v_col1 type = RAM_2P impl = LUTRAM latency = 1
#pragma HLS BIND_STORAGE variable = v_col2 type = RAM_2P impl = LUTRAM latency = 1
    ...
    // Initialize variables to avoid synthesis errors
    w_in = 0; x_in = 0; y_in = 0; z_in = 0;
    w_int1 = 0; x_int1 = 0; y_int1 = 0; z_int1 = 0;
    w_int2 = 0; x_int2 = 0; y_int2 = 0; z_int2 = 0;
    typename SVDTraits::SIntType w_out, x_out, y_out, z_out;
    typename SVDTraits::UIntType uw_in, ux_in, uy_in, uz_in;
    typename SVDTraits::UIntType uw_out, ux_out, uy_out, uz_out;
    typename SVDTraits::VIntType vw_in, vx_in, vy_in, vz_in;
    ...
    // Initialize more variables to avoid synthesis errors
    w_out = 0; x_out = 0; y_out = 0; z_out = 0;
    uw_in = 0; ux_in = 0; uy_in = 0; uz_in = 0;
    uw_out = 0; ux_out = 0; uy_out = 0; uz_out = 0;
    vw_in = 0; vx_in = 0; vy_in = 0; vz_in = 0;
    vw_out = 0; vx_out = 0; vy_out = 0; vz_out = 0;
...
#pragma HLS BIND_STORAGE variable = A type = RAM_2P impl = BRAM latency = 1
    OutputType S[RowsA][ColsA];
#pragma HLS BIND_STORAGE variable = S type = RAM_2P impl = BRAM latency = 1
    OutputType U[RowsA][RowsA];
#pragma HLS BIND_STORAGE variable = U type = RAM_2P impl = BRAM latency = 1
    OutputType V[ColsA][ColsA];
#pragma HLS BIND_STORAGE variable = V type = RAM_2P impl = BRAM latency = 1
