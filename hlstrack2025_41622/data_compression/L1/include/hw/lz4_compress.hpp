...
static void lz4CompressPart2(hls::stream<uint8_t>& in_lit_inStream,
    ...
    // OPTIMIZATION: Pre-read to reduce critical path delay
...
#pragma HLS PIPELINE II = 1
        ...
        // Read data once at start of WRITE_TOKEN state
        if (readOffsetFlag) {
            nextLenOffsetValue = in_lenOffset_Stream.read();
            readOffsetFlag = false;
        }

        // OPTIMIZED: Direct extraction without intermediate variables
        if (next_state == WRITE_TOKEN) {
            ...
            // OPTIMIZED: Direct calculation to reduce latency
            inIdx += (uint32_t)match_length + (uint32_t)lit_length + 4;

            // Check end conditions - balanced condition checks for reduced latency
            bool cond1a = (match_length == 777);
            bool cond1b = (match_offset == 777);
            bool end_cond1 = cond1a && cond1b;
            
            bool cond2a = (match_offset == 0);
            bool cond2b = (match_length == 0);
            bool end_cond2 = cond2a && cond2b;
            ...
            // ALGORITHM OPTIMIZATION: Direct comparisons instead of ternary
            ...
            if (lit_length >= 15) {
                ...
                lit_token = (ap_uint<4>)lit_length;
                lit_length = 0;
            ...
            if (match_length >= 15) {
                ...
                match_token = (ap_uint<4>)match_length;
                match_length = 0;
                extra_match_len = false;
            }
            outValue.range(3, 0) = match_token;
            
            match_offset_reg = match_offset + 1;
            
        } else if (next_state == WRITE_LIT_LEN) {
            // ALGORITHM OPTIMIZATION: Direct comparison for timing
            if (lit_length >= 255) {
                outValue = 255;
                lit_length -= 255;
            } else {
                outValue = (ap_uint<8>)lit_length;
                next_state = WRITE_LITERAL;
                readOffsetFlag = false;
            }
            ...
            // OPTIMIZATION: Use pre-calculated value
            ...
            if (match_length >= 255) {
                ...
                outValue = (ap_uint<8>)match_length;
                next_state = WRITE_TOKEN;
                readOffsetFlag = true;
            }
        }
        
        // OPTIMIZATION: Simplified write condition
