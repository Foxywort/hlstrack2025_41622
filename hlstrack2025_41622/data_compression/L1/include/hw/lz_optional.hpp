...
#pragma HLS PIPELINE II = 1
        // OPTIMIZATION: Parallel shift register
...
#pragma HLS UNROLL
        ...
        // ALGORITHM OPTIMIZATION: Simple comparison without break
...
#pragma HLS PIPELINE II = 1
            ...
            // OPTIMIZATION: Simplified comparison
            compressd_dt compareValue = compare_window.data[0];
            uint8_t compareLen = compareValue.range(15, 8);
            best_match = !(match_length < compareLen);
 ...
 * OPTIMIZED VERSION for lower latency and better timing
...
#pragma HLS PIPELINE II = 1
    ...
    lz_booster: for (; nextVal.strobe != 0; ++iIdx) {
#pragma HLS PIPELINE II=1
        // Read operations - SINGLE CYCLE for timing
        ap_uint<32> inValue = lclBufStream.read();
        auto tmp = inStream.read();
        lclBufStream << nextVal.data[0];
        nextVal = tmp;
        
        // Match computations - SINGLE CYCLE
        uint8_t tCh = inValue.range(7, 0);
        uint8_t tLen = inValue.range(15, 8);
        uint16_t tOffset = inValue.range(31, 16);
        boostFlag = (tOffset < c_boosterOffsetWindow);
        
        ap_uint<8> match_ch = nextMatchValue.range(7, 0);
        uint32_t mem_idx = iIdx % c_boosterOffsetWindow;
        local_mem[mem_idx] = tCh;
        outFlag = false;
        
        // Flag and length updates - SINGLE CYCLE
        bool cond1 = matchFlag && (match_len < MAX_MATCH_LEN);
        bool cond2 = (tCh == match_ch);
        bool match_condition = cond1 && cond2;
        ...
        if (skip_len) {
            ...
            outValue = inValue;
            matchFlag = tLen ? boostFlag : false;
            skip_len = (tLen && !boostFlag) ? (tLen - 1) : 0;
        }
        
        // Output and next match prep - SINGLE CYCLE
        uint32_t match_mem_idx = match_loc % c_boosterOffsetWindow;
        nextMatchValue = local_mem[match_mem_idx];
        if (outFlag) {
            outStream << outStreamValue;
        }
    }
...
/**
 * @brief This module helps in improving the compression ratio.
 * OPTIMIZED VERSION for better timing
 * Finds a better match length by performing more character matches
 * with supported max match, while maintaining an offset window.
 * Booster offset Window template argument (default value is 16K)
 * internally consume BRAM memory to implement history window.
...
#pragma HLS dependence variable = local_mem inter false
        ...
        // OPTIMIZATION: Simplify boostFlag
        boostFlag = (tOffset < BOOSTER_OFFSET_WINDOW);
        
        ap_uint<8> match_ch = nextMatchValue.range(7, 0);
        // Optimized modulo operation
        uint32_t mem_idx = i % BOOSTER_OFFSET_WINDOW;
        local_mem[mem_idx] = tCh;
        outFlag = false;

        // Balanced complex condition check for reduced latency
        bool cond1 = matchFlag && (match_len < MAX_MATCH_LEN);
        bool cond2 = (tCh == match_ch);
        bool match_condition = cond1 && cond2;
        ...
        if (skip_len) {
            ...
            // OPTIMIZATION: Simplify flag logic
            matchFlag = tLen ? boostFlag : false;
            skip_len = (tLen && !boostFlag) ? (tLen - 1) : 0;
        }
        
        // Optimized modulo operation for array indexing
        uint32_t match_mem_idx = match_loc % BOOSTER_OFFSET_WINDOW;
        nextMatchValue = local_mem[match_mem_idx];
        if (outFlag) {
            outStream << outStreamValue;
        }
