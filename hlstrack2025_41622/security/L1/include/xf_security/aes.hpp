...
class aesTable {
   public:
    aesTable() {
#pragma HLS inline
#pragma HLS bind_storage variable=ssbox type=rom_nP impl=lutram
#pragma HLS bind_storage variable=iibox type=rom_nP impl=lutram
    }
...
class aesEnc {
   public:
    void updateKey(ap_uint<W> cipherkey) {
        (void)cipherkey; // unused in generic template
    }
    void process(ap_uint<128> plaintext, ap_uint<W> cipherkey, ap_uint<128>& ciphertext) {
        (void)plaintext; // unused in generic template
        (void)cipherkey; // unused in generic template
        (void)ciphertext; // unused in generic template
    }
};
