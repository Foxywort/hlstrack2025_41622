...
#pragma HLS bind_storage variable=roundIndex type=rom_2p impl=lutram
...
#pragma HLS bind_op variable=blkNumReg op=udiv impl=auto latency=20 // was DivnS
...
#pragma HLS loop_tripcount min = 1 max = 1 avg = 1
        ...
        ap_uint<128> blkNumReg = msgLen / sizeR;
        ap_uint<128> blkNum = blkNumReg + 1;
#pragma HLS bind_op variable=blkNumReg op=udiv impl=auto latency=200 // was DivnS
#if !defined(__SYNTHESIS__) && __XF_SECURITY_SHA3_DEBUG__ == 1
        std::cout << "blkNum = " << std::dec << blkNum << std::endl;
#endif
