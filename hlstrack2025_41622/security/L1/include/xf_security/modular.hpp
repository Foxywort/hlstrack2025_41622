...
#pragma HLS PIPELINE II=1
        ...
        // DSP48E1 optimization: Use DSP for critical add operations in modular multiplication
        ap_uint<N + 1> sum_temp = addA + addM;
...
#pragma HLS bind_op variable=s op=add impl=dsp
    ...
    if (s > opM) {
        s -= opM;
#pragma HLS bind_op variable=s op=sub impl=dsp
    }
    return s;
}
...
ap_uint<N> addMod(ap_uint<N> opA, ap_uint<N> opB, ap_uint<N> opM) {
    // DSP48E1 optimization: Use DSP for modular addition
...
ap_uint<N> subMod(ap_uint<N> opA, ap_uint<N> opB, ap_uint<N> opM) {
    // DSP48E1 optimization: Use DSP for modular subtraction
    ap_uint<N + 1> sum;
    if (opA >= opB) {
        sum = opA - opB;
#pragma HLS bind_op variable=sum op=sub impl=dsp
