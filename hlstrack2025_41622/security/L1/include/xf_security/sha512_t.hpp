...
#endif
                ap_uint<w> T1_a = h + BSIG1<w>(e);
#pragma HLS bind_op variable=T1_a op=add impl=dsp
                ap_uint<w> T1_b = T1_a + Ch<w>(e, f, g);
#pragma HLS bind_op variable=T1_b op=add impl=dsp
                ap_uint<w> T1_c = T1_b + K[t];
#pragma HLS bind_op variable=T1_c op=add impl=dsp
                ap_uint<w> T1 = T1_c + Wt;
...
#pragma HLS bind_storage variable=msg_64_strm type=fifo impl=lutram
    hls::stream<ap_uint<5> > len_64_strm("len_64");
#pragma HLS stream variable = len_64_strm depth = 32
#pragma HLS bind_storage variable=len_64_strm type=fifo impl=lutram
    hls::stream<ap_uint<65> > blk_strm("blk_strm");
#pragma HLS stream variable = blk_strm depth = 32
#pragma HLS bind_storage variable=blk_strm type=fifo impl=lutram
